...
function createBlocks(){
  blocksDiv.innerHTML="";
  for(let i=0;i<3;i++){
    const shape=SHAPES[Math.floor(Math.random()*Math.min(level+2,SHAPES.length))];
    ...
    block.style.gridTemplateColumns=`repeat(${shape[0].length},44px)`;
...
function canPlace(x,y,s){
  for(let r=0;r<s.length;r++)
    for(let c=0;c<s[r].length;c++)
      if(s[r][c]){
        const nx=x+c,ny=y+r;
        if(nx<0||ny<0||nx>=size||ny>=size||grid[ny*size+nx])return false;
      }
  return true;
}
...
/* CONTROLS */
startBtn.onclick=()=>startScreen.style.display="none";
